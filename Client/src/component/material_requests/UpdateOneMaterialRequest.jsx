...
import { useAuth } from "../../context/AuthContext"; //  Use your context here

function UpdateOneMaterialRequest() {
	const { userToken, loading: authLoading } = useAuth(); //  use context
	...
	useEffect(() => {
		if (mRData?.getOneMaterialRequest && allItems.length > 0) {
	...
	return (
		<div className="update-container">
			<form className="update-form" onSubmit={submit}>
				<h1 className="update-form-title">Update Material Request</h1>
					...
					{rows.map((row, idx) => {
						...
						return (
							<div className="update-form-row" key={idx}>
								<h3>Material Request Row {idx + 1}</h3>
								<Select options={brands} value={row.brand} onChange={(val) => handleRowChange(idx, "brand", val)} placeholder="Filter By Brand" isClearable />

								<input type="number" value={row.quantity} onChange={(e) => handleRowChange(idx, "quantity", e.target.value)} min={1} placeholder="Qty" />
				...
				<div className="form-action-btn">
					...
					<button className="form-submit-btn" type="button" disabled={loading || !isFormValid} onClick={() => setIsOpen(true)}>
						Approve
					</button>
				</div>

				{!isFormValid && <p style={{ color: "red" }}>Please fill out all required fields.</p>}
