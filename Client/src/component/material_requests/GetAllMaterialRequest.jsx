...
// 	// Fetch and set requests on query change
// 	useEffect(() => {
// 		if (data?.getAllMaterialRequests) {
// 			setMRequests(data.getAllMaterialRequests);
// 		}
...
// 	// Always sort by date first, then by approval
// 	// useEffect(() => {
// 	// 	if (!mRequests) return;

// 	// 	let sorted = [...mRequests].sort((a, b) => dayjs(b.addedDate).valueOf() - dayjs(a.addedDate).valueOf());

// 	// 	sorted = sorted.sort((a, b) => {
// 	// 		const aStatus = a?.approvalStatus?.isApproved ? 1 : 0;
// 	// 		const bStatus = b?.approvalStatus?.isApproved ? 1 : 0;
// 	// 		if (aStatus !== bStatus) {
// 	// 			return sortApprovalAsc ? aStatus - bStatus : bStatus - aStatus;
// 	// 		}
// 	// 		return 0; // same approval → keep date order
// 	// 	});

// 	// 	setFilteredMRequests(sorted);
// 	// }, [mRequests, sortApprovalAsc]);
...
// 			// First, sort by approval status (waiting = 0, approved = 1)
// 			if (aApproved < pivotApproved) {
// 				left.push(arr[i]);
// 			} else if (aApproved > pivotApproved) {
// 				right.push(arr[i]);
// 			} else {
// 				// Same approval status → sort by date descending
// 				if (dayjs(arr[i].addedDate).valueOf() > dayjs(pivot.addedDate).valueOf()) {
// 					left.push(arr[i]);
// 				} else {
// 					right.push(arr[i]);
...
// 	// 	// Then move approved requests to the bottom
// 	// 	sorted = sorted.sort((a, b) => {
// 	// 		const aApproved = a?.approvalStatus?.isApproved ? 1 : 0;
// 	// 		const bApproved = b?.approvalStatus?.isApproved ? 1 : 0;
// 	// 		return aApproved - bApproved; // approved = 1, waiting = 0 → approved at bottom
// 	// 	});
...
// 	useEffect(() => {
// 		if (!mRequests) return;
// 		const sorted = quickSortRequests(mRequests);
// 		setFilteredMRequests(sorted);
// 	}, [mRequests]);
...
// 				if (searchValue) {
// 					const filtered = applyFuse(updated, searchValue);
// 					setFilteredMRequests(filtered);
// 				} else {
// 					setMRequests(updated); // triggers useEffect sorting
// 				}
